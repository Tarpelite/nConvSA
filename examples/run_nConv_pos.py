...
def train(args, train_dataset, model, tokenizer):
    ...
    for _ in train_iterator:
        ...
        iter_bar = tqdm(train_dataloader, desc="Iter(cls_loss=X.XXX, pos_loss=X.XXX, lr=X.XXXXXXX)")
        for step, batch in enumerate(iter_bar):
            ...
            tr_loss += loss.item()
            iter_bar.set_description("Iter (cls_loss=%5.3f, pos_loss=%5.3f, lr=%9.7f)" % (cls_loss.item(), pos_loss.item(), scheduler.get_lr()[0]))
    ...
    for eval_task, eval_output_dir in zip(eval_task_names, eval_outputs_dirs):
        ...
        for batch in tqdm(eval_dataloader, desc="Evaluating"):
            ...
            with torch.no_grad():
                ...
                outputs = model(**inputs)
                tmp_eval_cls_loss, tmp_eval_pos_loss, cls_logits, pos_logits = outputs[:4]

                tmp_eval_loss = tmp_eval_cls_loss +  tmp_eval_pos_loss
    ...
    for eval_task, eval_output_dir in zip(eval_task_names, eval_outputs_dirs):
        ...
        for batch in tqdm(eval_dataloader, desc="Prediction"):
            ...
            with torch.no_grad():
                ...
                outputs = model(**inputs)
                cls_logits, pos_logits = outputs[:2]
...
def load_and_cache_examples(args, task, tokenizer, evaluate=False, test=False):
    ...
    part = "train"
    get_func = processor.get_pos_train_examples
    if evaluate:
        part = "evaluate"
        get_func=processor.get_pos_dev_examples
    if test:
        part = "test"
        get_func = processor.get_pos_test_examples
